...
def get_new(se, proxy: dict = None, num: int = 0, user_id: str = '') -> set:
    """
...
def fetcher(pid: str = None, pname: str = None, uid: str = None, uname: str = None):
    """
    Fetch illustration info out of database. Run it in thread.
    At least one parameter must be passed in.
    If pid specified, the other args are ignored.
    """
    try:
        ...
        if pid:  # If pid specified, the other args are ignored
            select_str = 'ILLUSTID = ' + pid
        else:
            if pname:
                select_str = 'ILLUSTTITLE GLOB \'' + pname + '\''
            if uid:
                if select_str:
                    select_str += ' AND '
                select_str = select_str + 'USERID = ' + uid
            if uname:
                if select_str:
                    select_str += ' AND '
                select_str = select_str + 'USERNAME GLOB \'' + uname + '\''
        if select_str:
            print(select_str)
